...
export interface CheckoutState {
  ...
  // Origin — which flow opened this checkout session
  origin: CheckoutOrigin;
  /** Resolved cart lines when origin === "cart"; empty for the Studio flow */
   ...
   *  skips the variants step and opens directly on shipping. */
  
  startCheckoutFromCart: (lines: CartCheckoutLine[]) => void;
  reset: () => void;
  goBack: () => void;
  goForward: () => void;
  saveDraft: () => void;
  ...
  /** Subtotal (excludes shipping) — sums cart line totals when
   *  origin === "cart", otherwise (basePrice + printCost) × qty. */
  getSubtotal: () => number;
...
const initial = {
  ...
  direction: 1,
  origin: "studio" as CheckoutOrigin,
  cartItems: [] as CartCheckoutLine[],
  productId: "",
  productName: "",
...
export const useCheckoutStore = create<CheckoutState>((set, get) => ({
  ...
  startCheckoutFromCart: (lines) => {
    ...
    set({
      ...
      direction: 1,
      cartItems: lines,
      currencySymbol: lines[0]?.currencySymbol ?? "Br",
    });
  },
  ...
  reset: () => {
    ...
    set({
      ...initial, // also resets origin -> "studio" and cartItems -> []
  ...
  getTotalQuantity: () => {
    const { origin, cartItems, selectedVariants } = get();
    if (origin === "cart") {
      return cartItems.reduce((s, i) => s + i.quantity, 0);
    }
    return Array.from(selectedVariants.values()).reduce((s, v) => s + v.quantity, 0);
  },

  getSubtotal: () => {
    const { origin, cartItems, basePrice, printCost } = get();
    if (origin === "cart") {
      return cartItems.reduce((s, i) => s + i.unitPrice * i.quantity, 0);
    }
    return (basePrice + printCost) * get().getTotalQuantity();
  },

  getOrderItems: (): CheckoutItem[] => {
    const { origin, cartItems, productId, selectedVariants, artworks, printAreas, basePrice, printCost } = get();

    if (origin === "cart") {
      // Each cart line is already a resolved product+color+size+qty — no
      // print areas/artwork since these are ready-made marketplace items.
      return cartItems.map((line) => ({
        productId: line.productId,
        variantId: `${line.productId}:${line.colorName}:${line.size}`,
        size: line.size,
